...
import {
  ArrowRight, CheckCircle2, Plus, Trash2, Upload, Info, GraduationCap,
  BookOpen, Mic2, FileText, Home, PlaneTakeoff,
} from "lucide-react";
...
function useServiceForm(serviceType: string, onClose: () => void) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await fetch(apiUrl("/leads/service-request"), { 
        method: "POST", 
...
export function ServicesGrid() {
  ...
  return (
    <>
      <section id="services" aria-label="Our Services" className="py-12 md:py-18 bg-muted/40">
