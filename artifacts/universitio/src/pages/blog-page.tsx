          ...
          <div className="mb-16 pb-12 border-b border-border">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              {blogCategories.map((cat) => {
                ...
                return (
                  <Link key={cat.slug} href={isActive ? `/blog/category/${cat.slug}` : '#'}>
                    <button 
                      disabled={!isActive}
                      className={`w-full px-4 py-2.5 rounded-lg text-sm font-medium transition-all text-center ${
                        isActive 
                          ? 'bg-slate-100 text-foreground border border-border hover:border-primary hover:bg-primary/5 hover:text-primary cursor-pointer' 
                          : 'bg-slate-50 text-muted-foreground border border-slate-200 cursor-not-allowed opacity-50'
                      }`}
                    >
