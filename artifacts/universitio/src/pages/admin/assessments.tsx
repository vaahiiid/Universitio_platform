...
const STATUSES = ["New", "Reviewed", "Contacted", "In Progress", "Closed", "Archived"];
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    ...
    Contacted: "bg-green-100 text-green-700", "In Progress": "bg-purple-100 text-purple-700",
    Closed: "bg-gray-100 text-gray-600", Archived: "bg-gray-100 text-gray-500",
  };
...
function ScoreBadge({ score, band }: { score: number; band: string }) {
  ...
  return <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-bold ${color}`}>{score}% — {band}</span>;
}
...
function DetailView({ id }: { id: number }) {
  ...
  useEffect(() => {
    apiFetch<Record<string, unknown>>(`/admin/assessments/${id}`)
      ...
      .catch(() => navigate("/admin/assessments"))
  ...
  const strengths = data.additionalStrengths as string[] | null;

  return (
    <AdminLayout>
      <div className="max-w-3xl">
        <button onClick={() => navigate("/admin/assessments")} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-4">
...
function ListView() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await apiFetch<{ data: Record<string, unknown>[]; pagination: Pagination }>(`/admin/assessments?${params}`);
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          {loading ? (
            ...
            <div className="py-16 text-center text-sm text-muted-foreground">No assessments found.</div>
          ) : (
...
export default function AssessmentsPage() {
  const [match, params] = useRoute("/admin/assessments/:id");
